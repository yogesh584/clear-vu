...
const GarmentsComp = ({ activeTab, isDataAlreadyFetched, changeLinenStatus }) => {
    ...
    useEffect(() => {
        if (activeTab == "garments") {
            if (!isDataAlreadyFetched.card) {
                requestGarmentsCards("get", `api/inventory/get-summaryCard?userId=${userId}&categoryId=2&page=0&size=0`);
            }

            if (!isDataAlreadyFetched.table) {
                requestGarmentsData("get", `api/inventory/management?userId=${userId}&categoryId=2&size=${10}&page=0`);
            }
        }
    }, [activeTab])
    ...
    const sortingHandler = (sortBy) => {
        ...
        if (currentSort.sortBy === sortBy) {
            ...
            requestGarmentsData("get", `api/inventory/management?userId=${userId}&categoryId=3&size=${perPage}&page=${1}&orderByField=${finalSortField}&ascending=${newOrder == "asc" ? true : false}`);
            setCurrentSort({ sortBy, order: newOrder });
        } else {
            requestGarmentsData("get", `api/inventory/management?userId=${userId}&categoryId=3&size=${perPage}&page=${1}&orderByField=${finalSortField}&ascending=${currentSort.order == "asc" ? true : false}`);
    ...
    const fetchMoreData = ({ selected }) => {
        setPage(selected);
        requestGarmentsData("get", `api/inventory/management?userId=${userId}&categoryId=2&size=${perPage}&page=${selected}`);
    };
    ...
    const perPageChangeHandler = (event) => {
        ...
        requestGarmentsData("get", `api/inventory/management?userId=${userId}&categoryId=2&size=${records_per_page}&page=1`);
    };
